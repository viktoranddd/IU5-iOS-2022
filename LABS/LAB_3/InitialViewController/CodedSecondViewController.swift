...
final class CodedSecondViewController: UIViewController {
    ...
    private func registerCell() {
        tableViewController.register(CodedSecondViewControllerCell.self, forCellReuseIdentifier: "CodedSecondViewControllerCell")
    }
    
}

extension CodedSecondViewController: UITableViewDelegate {
    func tableView(_ tableView: UITableView, heightForRowAt indexPath: IndexPath) -> CGFloat {
        return 70
    }
    
    func tableView(_ tableView: UITableView, didSelectRowAt indexPath: IndexPath) {
        let codedThirdViewController = CodedThirdViewController()
        codedThirdViewController.view.backgroundColor = .white
        navigationController?.pushViewController(codedThirdViewController, animated: true)
        tableView.deselectRow(at: indexPath, animated: true)
    }
}

extension CodedSecondViewController: UITableViewDataSource {
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        guard let cell = tableViewController.dequeueReusableCell(withIdentifier: "CodedSecondViewControllerCell", for: indexPath) as? CodedSecondViewControllerCell else {
            fatalError()
        }
